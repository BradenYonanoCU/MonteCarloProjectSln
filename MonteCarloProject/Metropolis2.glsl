...
mat4 RayCast(vec3 rayD, vec3 rayO, vec3 lightO){
    ...
    vec3 sphereLoc = vec3(-2., -1., 0.4);

    //First ray trace the sphere
    vec4 rayPoint = pointRayWithRadius(rayD, rayO, sphereLoc, 1.);

    vec3 largeGlassD, largeGlassP, largeGlassN;
    largeGlassD = rayD;
    largeGlassP = rayO;
    RaycastGlassSphere(largeGlassD, largeGlassP, largeGlassN, sphereLoc, .7);
    rayPoint.xyz = largeGlassP;
    ...
    glassD = rayD;
    glassP = rayO;
    RaycastGlassSphere(glassD, glassP, glassN, vec3(-2., .0, -.5), .5);
    ...
    float dSph = distance(rayO, largeGlassP);
    ...
    if(abs(minD - dSph) < 0.05){
        ...
        */
        ...
        newRayD = mix(largeGlassD, DiffuseMaterialBounce(largeGlassN, largeGlassD), .01);
        newRayO = largeGlassP + newRayD * .001;
        newNormal = largeGlassN;
